(function() {
    ...
    // ---------------------------------------------------------------
    //  FULL-VIEWPORT CANVAS (no gaps, no scrolling)
    // ---------------------------------------------------------------
    const MAX_DPR = 2;
    let W, H, DPR;
    ...
    window.addEventListener('resize', () => {
        ...
        pipes.forEach(p => { p.x = (p.x / prevW) * W; p.width = PIPE_W; p.capH = PIPE_CAP_H; });
        coins.forEach(c => { c.x = (c.x / prevW) * W; });
        updateDimensions();
    });
    ...
    let pipes = [], particles = [], clouds = [], stars = [], coins = [];
    let groundOffset = 0, shakeMag = 0, flashAlpha = 0, timeOver = null;
    let frameCount = 0, comboCount = 0, comboTimer = 0, streakCount = 0, levelMsgTimer = 0;
    ...
    function spawnPipe(x) {
        ...
        pipes.push({
            ...
            width: PIPE_W, capH: PIPE_CAP_H, passed: false, coinSpawned: false
        });
    }
    function resetPipes() {
        pipes = [];
    ...
    // ---------------------------------------------------------------
    function startGame() {
        resetBird(); resetPipes(); coins = [];
        score=0; currentLevel=1; comboCount=0; comboTimer=0; streakCount=0;
        levelMsgTimer=0; particles=[]; groundOffset=0; shakeMag=0; flashAlpha=0; timeOver=null;
        gameMode = STATE.RUN;
    ...
    function update(dt){
        ...
        if(gameMode===STATE.PRE){
            ...
            coins.forEach(coin => {
                ...
                if (dist < BIRD_R + coin.r) {
                    coin.collected = true;
                    score += 2;
                    hudScore.textContent = score;
                    emit(coin.x, coin.y, 8, ['#ffd700','#fff','#ffd700'], 4, 0.4);
                }
            });
    ...
    function init(){
        ...
        resetBird(); resetPipes(); genStars(); genClouds();
        coins=[]; particles=[]; score=0; currentLevel=1; gameMode=STATE.PRE;
